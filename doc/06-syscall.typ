...
`sys_mmap`：该系统调用允许进程在其虚拟地址空间中映射内存区域。支持选项包括指定起始地址、长度、权限（`PROT_READ`、`PROT_WRITE` 和 `PROT_EXEC`）和标志（`MAP_SHARED`、`MAP_PRIVATE`、`MAP_FIXED`、`MAP_ANONYMOUS` 和 `MAP_NORESERVE`）。MankorOS 通过 `LightProcess` 的内存管理器来分配和映射物理页框，并将这些页框映射到进程的虚拟地址空间中。
...
除了上述的系统调用外，MankorOS 还实现了四个系统调用，分别是三个时间相关的 `gettimeofday`、`times` 和 `nanosleep` 
以及一个系统信息相关的 `uname`。
