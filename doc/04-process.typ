...
-  若进程执行某个耗时久的系统调用 (比如 `sleep`), 代表它的 `Future` 会直接返回 Pending, 从而使它离开调度队列;直到阻塞的条件被满足后，它会被 waker 自动重新加入回调度队列。因此不需要代表 "进程因为缺少某条件不能被调度" 的状态。
...
`RangeMap` 的实现直接借用了 #link("https://gitee.com/ftl-os/ftl-os/blob/master/code/kernel/src/tools/container/range_map.rs")[FTL-OS] 的实现。但额外增加了原地修改区间长度的 `extend_back` 和 `reduce_back` 方法，以针对堆内存的动态增长和缩减进行优化。对于其他类型的区间长度增减，仍然采用 "创建新区间 - 合并" 和 "分裂旧区间 - 删除" 的方式。
...
目前 MankorOS 中的进程调度器是一个简单的 FIFO 调度器，其内部维护了一个双端队列，依次从队列头部取出包含待调度进程的 `Future` 并执行。当用户程序因为时间片用完而返回内核时，放弃该轮执行并被重新加入调度队列的尾部。当用户程序因为系统调用而返回内核，并且该系统调用会 "阻塞" 时，它会返回一个 `Pending` 状态，直到等待到阻塞条件满足后被 waker 唤醒。换而言之，异步内核中没有 "阻塞" 的概念，一切操作要么马上结束，要么放弃执行等待回调。
